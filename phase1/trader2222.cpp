...
int main(int argc, char* argv[]) {
    ...
    for(int j = 0; j < all_trades2.size(); j++){
    ...
    for(int j = 0; j < all_company_names2.size(); j++) stock_names2.push_back(all_company_names2[j]);
    ...
    for (int j = previous_size ; j < all_trades2.size() ; j ++ ) {
        ...
        for (int k = 0 ; k < j ; k ++) {
            if (all_trades2[j].action == "b") {
                if (all_trades2[k].participating && same_trade_line_detector2(j, k)) {
                    ...
                    if (all_trades2[k].participating && cycle_detect2(pair_checker)) {
