...
int main(int argc, char* argv[]) {
    ...
    for(int j = 0; j < previous_size; j++){
        for(int g = all_names_prevSize2; g < all_company_names2.size(); g++){
            all_trades2[g].mapped_stock_quantity.push_back(0);
        }
    }
    for(int j = previous_size; j < all_trades2.size(); j++){
    ...
    for (int j = previous_size ; j < all_trades2.size() ; j ++ ) {
        ...
        for (int k = 0 ; k < j ; k ++) {
            if (all_trades2[j].action == "b") {
                if (all_trades2[k].participating && same_trade_line_detector2(j, k) && all_trades[k].action == all_trades[j].action) {
                    if (all_trades2[j].package_price >= all_trades2[k].package_price) all_trades2[k].participating = false;
                    else all_trades2[j].participating = false;
                    break;
                }
                else {
                    ...
                    if (all_trades2[k].participating && same_trade_line_detector2(j, k) && all_trades[k].package_price==all_trades[j].package_price && cycle_detect2(pair_checker)) {
    ...
    for(int j = 0; j < previous_size; j++){
        for(int g = all_names_prevSize; g < all_company_names.size(); g++){
            all_trades[g].stock_quantity.push_back(0);
        }
    }
