...
int maximum_individual_stock_quantity = 10;
...
void same_structure_cancelation(int i, int j) {
    if (all_trades_trader[i].participating == false || all_trades_trader[j].participating == false) return;
    if (all_trades_trader[i].action == all_trades_trader[j].action) return;
    if (all_trades_trader[i].package_price != all_trades_trader[j].package_price) return;
    for (int k = 0 ; k < stock_names_trader.size() ; k ++) {
        if (all_trades_trader[i].stock_quantity[k] != all_trades_trader[i].stock_quantity[k]) return;
    }
    if (all_trades_trader[i].total_quantity > all_trades_trader[j].total_quantity) {
        all_trades_trader[j].participating = false;
        all_trades_trader[i].total_quantity -= all_trades_trader[j].total_quantity;
        all_trades_trader[j].total_quantity = 0;
    } 
    else if(all_trades_trader[i].total_quantity < all_trades_trader[j].total_quantity) {
        all_trades_trader[i].participating = false;
        all_trades_trader[j].total_quantity -= all_trades_trader[i].total_quantity;
        all_trades_trader[i].total_quantity = 0;
    }
    else {
        all_trades_trader[i].total_quantity = 0;
        all_trades_trader[j].total_quantity = 0;
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int l = prevUserSize ; l < all_user_trades.size() ; l ++ ) {
        ...
        outputString += to_string(all_user_trades[l].start_time) +  " " + all_user_trades[l].brokerName + " ";
